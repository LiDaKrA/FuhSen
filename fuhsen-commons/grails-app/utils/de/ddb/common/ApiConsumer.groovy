...
import de.ddb.common.exception.ForbiddenException
...
class ApiConsumer {
     ...
     */
    static def getText(String baseUrl, String path, boolean httpAuth = false, optionalQueryParams = [:], optionalHeaders = [:], fixWrongContentTypeHeader = false, alreadyEncodedQuery = false, isDDBClient = true) {

        def queryParams = [:]

        if (isDDBClient){
            queryParams["client"] = DDBNEXT_CLIENT_NAME
        }

        queryParams.putAll(optionalQueryParams)

        if(fixWrongContentTypeHeader){
            return requestServer(baseUrl, path, queryParams, Method.GET, ContentType.JSON, null, httpAuth, optionalHeaders, true, null, alreadyEncodedQuery)
        }else{
            return requestServer(baseUrl, path, queryParams, Method.GET, ContentType.TEXT, null, httpAuth, optionalHeaders, false, null, alreadyEncodedQuery)
        }
    }
     ...
     * @return An ApiResponse object containing the server response
     */
    static def getJson(String baseUrl, String path, boolean httpAuth = false, optionalQueryParams = [:], optionalHeaders = [:], alreadyEncodedQuery = false, isDDBClient = true) {
        def queryParams = [:]
...
//        }
        ...
        return requestServer(baseUrl, path, queryParams, Method.GET, ContentType.JSON, null, httpAuth, optionalHeaders, false, null, alreadyEncodedQuery)
    }
     ...
     * @return An ApiResponse object containing the server response
     */
    static def postJson(String baseUrl, String path, boolean httpAuth = false, postParameter, optionalQueryParams = [:], optionalHeaders = [:], alreadyEncodedQuery = false , isDDBClient = true) {
        def queryParams = [:]
        ...
        return requestServer(baseUrl, path, queryParams, Method.POST, ContentType.JSON, postParameter.toString(), httpAuth, optionalHeaders, false, null, alreadyEncodedQuery)
    }
     ...
     * @return An ApiResponse object containing the server response
     */
    static def putAny(String baseUrl, String path, boolean httpAuth = false, putParameter, optionalQueryParams = [:], optionalHeaders = [:], alreadyEncodedQuery = false, isDDBClient = true) {
        def queryParams = [:]
        ...
        queryParams.putAll(optionalQueryParams)
        return requestServer(baseUrl, path, queryParams, Method.PUT, ContentType.URLENC, putParameter, httpAuth, optionalHeaders, false, null, alreadyEncodedQuery)
    }
     ...
     * @return An ApiResponse object containing the server response
     */
    static def putJson(String baseUrl, String path, boolean httpAuth = false, putParameter, optionalQueryParams = [:], optionalHeaders = [:], alreadyEncodedQuery = false, isDDBClient = true) {
        def queryParams = [:]
        ...
        queryParams.putAll(optionalQueryParams)
        return requestServer(baseUrl, path, queryParams, Method.PUT, ContentType.JSON, putParameter.toString(), httpAuth, optionalHeaders, false, null, alreadyEncodedQuery)
    }
     ...
     * @return An ApiResponse object containing the server response
     */
    static def getAny(String baseUrl, String path, boolean httpAuth = false, optionalQueryParams = [:], optionalHeaders = [:], alreadyEncodedQuery = false, isDDBClient = true) {
        def queryParams = [:]
        ...
        return requestServer(baseUrl, path, queryParams, Method.GET, ContentType.ANY, null, httpAuth, optionalHeaders, false, null, alreadyEncodedQuery)
    }
    ...
    private static def requestServer(baseUrl, path, query, method, content, requestBody, boolean httpAuth = false, optionalHeaders, fixWrongContentTypeHeader, OutputStream streamingOutputStream, alreadyEncodedQuery = false) {
        ...
        //Normalize the parameters baseurl and path to avoid problems with the HTTPBuilder:
        //Always use RELATIVE paths (without leading '/') because ABSOLUTE paths can cause problems with baseurls that itself contains a path, e.g http://ddb-fiz-frontend.iais.fraunhofer.de/cortex/api/
        //In such a case the HTTPBuilder throw away the path (cortex/api/) from the baseUrl
        baseUrl = (baseUrl.endsWith("/")) ? baseUrl : (baseUrl+ "/")
        if (path) {
            path = (path.startsWith("/")) ? path.substring(1) : path
        }

        //API urls need some special handling
        path = fixPathForApiContext(baseUrl, path)
        ...
        try {
			...
			http.auth.basic "collarad", "s2F5h8K1AK1ZhA5xo9"

            def apiKey = "123456"//setApiKey(optionalHeaders, baseUrl)
            ...
            http.request(method, content) { req ->
				...
				//log.info("alreadyencoded: "+alreadyEncodedQuery+" query: "+query)
                ...
                response.'403' = { resp ->
                    return build403Response(timestampStart, uri.toString(), method.toString(), content.toString(), resp.headers, "Server answered 403 -> " + uri.toString() + " / " + resp.headers.'Error-Message', requestBody, apiKey)
                }
    ...
    /**
     * Utility method to build the ApiResponse object for 403 responses
     * @param timestampStart The timestamp when the request was send
     * @param calledUrl The complete URL that was called
     * @param method The request method (Method.GET, Method.POST)
     * @param content The expected response content (ContentType.TEXT, ContentType.JSON, ContentType.XML, ContentType.BINARY)
     ...
     * @return An ApiResponse object containing the response information
     */
    private static def build403Response(timestampStart, calledUrl, method, content, responseHeader, exceptionDescription, postBody, apiKey){
        def duration = System.currentTimeMillis()-timestampStart
        ForbiddenException exception = new ForbiddenException(exceptionDescription)
        def response = new ApiResponse(calledUrl, method.toString(), content, "", duration, exception, ApiResponse.HttpStatus.HTTP_403, responseHeader, postBody, apiKey)
        log.info response.toString()
        return response
    }
    ...
    /**
     * This fix allows to run two ddb-next applications to run on the same tomcat under different contexts.
     * @param baseUrl the basurl of the tomcat
     * @param path the (partial) request path
     * @return the context modified path
     */
    static def fixPathForApiContext(String baseUrl, String path) {
    ...
    static def setAuthHeader(http){
        try {
            ...
            log.error "setAuthHeader(): Could not get header-data from session"
        }
    }

    private static String createQueryStringFromMap(query) {
        String out = ""
        if (query) {
            query.each {
